...
class Ban extends BaseCommand {
    ...
    async run(e) {
        ...
        const reason = e.options.getString("reason") ? e.options.getString("reason") : e.translate("NO_REASON").replace("{user}", e.user.username);
        ...
        e.guild.members.kick(`${user.id}`, reason)
            .then(() => {
